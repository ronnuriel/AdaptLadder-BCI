...
\]
Here $\epsilon$ is a small stabilizer. We then compare this distance to each past source session and reuse the input layer of the closest past session:
\[
...
Table~\ref{tab:operational-ladder} summarizes the deployment interpretation of these results. The ladder is not a fixed sequence of updates to apply every day. Instead, the system should first ask whether the previous input state is still recent and plausible, then use geometry to retrieve older candidates when recency is stale or questionable, and only then use supervised calibration as an escalation step when stored states are unreliable.

\begin{table*}[t]
\centering
\caption{Operational interpretation of the adaptation ladder. PER values are evidence from this audit rather than guaranteed deployment performance. Lower PER is better.}
\scriptsize
\setlength{\tabcolsep}{3pt}
...
Stale previous state ($>$14 days) & Reuse latest state anyway & \textbf{Check geometry and retrieve a stored candidate} & Previous degrades to 24.54\% PER; geometry $K=20$ reaches 12.74\% overall & The previous layer has a time horizon. Geometry should be used as a retrieval signal when recency is no longer trustworthy. \\
No reliable stored state & Full retraining or blind reuse & \textbf{Residual input calibration} ($K=20$ labeled trials) & Fixed middle: 22.67\% $\rightarrow$ 18.59\% PER & Calibration helps, but remains far from native-day performance; use it as an escalation step. \\
Simple distribution shift & Mean/scale correction & \textbf{Do not rely on moment matching} & Channel-wise moment match: 22.73\% PER & Channel-wise mean/variance alignment does not capture the decoder-relevant drift. \\
Geometry-selected older candidate & Always override previous & \textbf{Use a confidence gate} & At $K=20$, older selected in 11 sessions but wins in 3 & Geometry retrieves plausible candidates, but covariance distance alone is not reliable enough as a standalone policy. \\
\bottomrule
\end{tabular}
\label{tab:operational-ladder}
\end{table*}

This operational view clarifies why geometry remains useful even though it does not outperform previous-session reuse on average. Recency is the default policy, but it has a limited time horizon. Geometry is valuable as a retrieval and warning signal when the previous input state is stale or geometrically implausible, while supervised calibration is reserved for cases where no stored state appears reliable.
