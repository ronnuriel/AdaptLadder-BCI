...
\textbf{Neural feature geometry.} In this work, neural geometry means the shape of the neural population activity in feature space. Each time bin from each trial is treated as one neural-feature vector, so a session becomes a cloud of points in channel space. The covariance matrix summarizes how neural channels vary together, while subspace statistics summarize the dominant directions of population activity. Intuitively, two sessions are geometrically close if their neural activity clouds have similar shape, even if the sessions occur on different days. For K-shot source selection, the first $K$ unlabeled target trials are used to estimate the target covariance. We use relative Frobenius covariance distance:
\[
d_{\mathrm{cov}}(t,s)
=
\frac{
