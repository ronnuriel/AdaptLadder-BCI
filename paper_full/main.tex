...
\]
Here $\epsilon=10^{-8}$ is a small stabilizer to prevent division by zero. We then compare this distance to each past source session and reuse the input layer of the closest past session:
\[
...
Stale previous state ($>$14 days) & Reuse latest state anyway & \textbf{Check geometry and retrieve a stored candidate} & Previous degrades to 24.54\% PER; geometry $K=20$ reaches 12.74\% overall & The previous layer has a time horizon; geometry is useful when recency is no longer trustworthy. \\
