...
Intracortical communication BCIs have advanced from handwriting-based brain-to-text decoding~\cite{willett2021handwriting,fan2023corp} to speech neuroprostheses that decode attempted speech~\cite{willett2023speech,card2024accurate}. Recent work also explores stronger speech backbones and pretrained acoustic representations for neural decoding~\cite{fiedler2025wav2vecbrain}. Our work is complementary: given an already trained brain-to-text decoder with day-specific input layers, we ask how to keep the decoder usable across sessions through lightweight input-side adaptation rather than retraining the backbone.

Cross-session stability is often studied through neural geometry and alignment. Manifold-alignment methods can stabilize fixed decoders across recording instabilities~\cite{degenhart2020stabilization}, and dynamics-aware approaches such as NoMAD use temporal structure for unsupervised alignment over weeks to months~\cite{karpowicz2025nomad}. Speech BCI work also explores adversarial session-invariant learning~\cite{zhang2026align} and continual recalibration with pseudo-labels~\cite{fan2023corp}. We do not propose a new invariant decoder or full recalibration method. Instead, we audit low-cost choices available before retraining: moment correction, lightweight input calibration, previous-session reuse, and geometry-guided source-layer retrieval.
...
The main claims above are decoder-facing T15 results. We use T12 and inner-speech data only as geometry-only supporting checks. In T12 diagnostic blocks, covariance distance increases with temporal separation for spike-power features (Spearman $\rho=0.44$, $p=5.4\times10^{-7}$), and threshold-crossing variants tx1--tx4 show the same positive trend ($\rho=0.34$--$0.47$, all $p<1.3\times10^{-4}$). Geometry selects an older non-previous source in 6--9 of 15 targets depending on feature type, including a 53-day lag in one spike-power case. This supports the idea that recency is important but not sufficient to describe longitudinal neural geometry, without claiming T12 decoder replication.

As a separate behavior-state check, we analyzed interleaved verbal-behavior blocks from the T12/T15/T16/T17 inner-speech dataset~\cite{kunz2025inner}. We did not train a decoder, evaluate PER, or attempt privacy-sensitive inner-speech decoding. Within each participant, we grouped activity by speech behavior and asked whether covariance geometry retrieves the matching behavior. The strongest setting, binned threshold crossings during the go epoch, reached 25/26 nearest-behavior accuracy (96.15\% vs. 20\% chance), with between-behavior distances 2.60 times within-behavior distances. This preliminary check suggests that input states may reflect behavior mode as well as recording day.

\section{Discussion}
The main lesson is that cross-session adaptation should not be treated as a single fixed correction. Wrong input states cause large decoder-facing degradation, moment matching does not emulate the learned day-specific input layers, and learned input calibration recovers only part of the lost performance. The most useful low-cost intervention is to choose a compatible input state while keeping the GRU decoder frozen.

The resulting deployment view is conservative. A new day should first try the previous input state, because it is the strongest simple default when it is truly recent. Geometry should then be used to retrieve older candidates only when short-window covariance/subspace structure suggests a better match. If no stored state is reliable and labels are available, residual input calibration can update only the input transform. Table~\ref{tab:deployment-ladder} summarizes this interpretation: choose the lightest action supported by the beginning-of-day evidence.
...
Geometry is therefore best viewed as a retrieval signal, not as a standalone policy. The failed distance-ratio and stump gates show that covariance distance alone is too weak to safely override recency. The drift-fingerprint analysis points to a better direction: older states are useful when they reduce subspace mismatch. A future online policy should combine temporal gap, subspace margin, source rank, session quality, decoder confidence, and possibly hidden-state or logit-space diagnostics. Preliminary alignment probes should also be treated cautiously: geometry-based feature transformations may introduce harms, so geometry is currently more reliable for retrieval than for automatic feature transformation.

A small number of difficult sessions dominate worst-case PER, especially 2025-03-30 and 2025-03-14. Sensitivity checks excluding the worst one or two sessions preserve the qualitative ordering, so we retain all sessions and treat these days as stress cases rather than exclusions. Finally, the geometry-only T12 and inner-speech checks suggest that the same state-library view may extend beyond T15 recording days to other participants and behavior modes, but decoder-facing replication, WER evaluation, and closed-loop adaptation remain future work.
