...
The clearest next step is a confidence-aware policy that decides when to reuse,
retrieve, or calibrate. Our simple gates show that covariance distance alone is
too weak as a decision rule, while the drift fingerprints suggest that subspace
mismatch is a more informative signal for older-state retrieval. A stronger
policy should combine temporal gap, subspace margin, source rank, decoder
confidence, session quality, and possibly hidden-state or logit-space features.
A second direction is to evaluate the same ladder with word error rate and a
language-model decoding stack, rather than greedy PER alone. Finally, the
geometry-only T12 and inner-speech analyses should be upgraded, when possible,
to decoder-facing evaluations. These extensions would test whether the same
input-state view holds across participants, tasks, and speech modes.
