...
Operationally, the ladder has three deployment regimes. If the previous state is recent, reuse it without labels: previous-source PER is 9.97\% when the last input state is 0--3 days old. If the previous state is stale, geometry should be used as a retrieval signal: previous-source PER degrades to 24.54\% beyond two weeks, while K-shot geometry reaches 12.74\% overall at $K=20$. If no stored state appears reliable, residual input calibration is the fallback, improving the fixed-middle stress setting from 22.67\% to 18.59\% PER. Because geometry-selected older candidates win only 3/11 times at $K=20$ and false-positive retrievals cost 3.23 percentage points on average, any override should be gated by confidence features.
