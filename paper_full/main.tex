...
\caption{Recency-aware adaptation ladder for cross-session BCI drift. At the start of a new target session, the first $K$ trials are used to estimate neural feature geometry. The target geometry is compared to a library of past input states. The system first reuses the previous input layer as the default, retrieves an older candidate when geometry suggests a closer match, or performs lightweight input-layer calibration when stored states are unreliable. The shared GRU decoder and output head remain frozen throughout.}
...
preliminary alignment direction is also worth treating cautiously: geometry-based
feature transformations, such as covariance alignment, may introduce harms, so
geometry is currently more reliable as a retrieval signal than as an automatic
feature transformation. Another direction is to test whether the input-state library view extends beyond
